...
func (r *SceneRenderer) deliverEventsToTree(shadow *shadowNode) {
	...
	if inputNode, ok := shadow.sceneNode.(sg.InputNode); ok {
		state, _ := shadow.state.(*sg.InputState)
		if state == nil {
			state = &sg.InputState{}
			shadow.state = state
		}
...
func (r *SceneRenderer) resolveTree(shadow *shadowNode, oldShadow *shadowNode) {
	...
	if newRenderableNode, ok := node.(sg.RenderableNode); ok {
		// Render node
		state := sg.RenderState{
			Window:    r.Window,
